...
Chrome grants `activeTab` only when the user invokes the extension action, not when a button inside an already-open side panel is clicked. SuiFill therefore handles the toolbar action explicitly: the same click grants temporary access to that page and opens the side panel. The user must invoke the toolbar action again after navigating to a different site.

M4 uses that temporary grant and `scripting` only when the unlocked user presses **Scan current page**. The scan payload excludes every field's current value and is not persisted.
